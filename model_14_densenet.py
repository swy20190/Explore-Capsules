...
import torch
# nn.functional is used to get the functions like relu.
import torch.nn.functional as F
# nn comprise of all neural network modules including the loss functions required
...
class DenseNet161(nn.Module):
    def __init__(self, train=False):
        ...
        self.densenet161 = self.DenseNet(models.densenet161(pretrained=True), 0, 18)
        if train:
            self.densenet161.train(mode=True)
            self.freeze_gradient()
        else:
            self.densenet161.eval()

    def DenseNet(self, densenet, begin, end):
        features = nn.Sequential(*list(densenet.features.children())[begin:(end+1)])
        return features

    def freeze_gradient(self, begin=0, end=9):
        for i in range(begin, end+1):
            self.densenet161[i].requires_grad = False

    def forward(self, input):
        return self.densenet161(input)    
...
class FeatureExtractorDense(nn.Module):
    def __init__(self):
        super(FeatureExtractorDense, self).__init__()

        self.conv0 = nn.Conv2d(in_channels=2208, out_channels=256, kernel_size=5, stride=2, padding=2)
